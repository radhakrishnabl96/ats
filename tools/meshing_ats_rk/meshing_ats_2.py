...
class Mesh3D(object):
    ...
    def write_exodus(self, filename, face_block_mode="one block"):
        ...
        #old_to_new_elems = sorted([(old,i) for (i,old) in enumerate(new_to_old_elems)], key=functools.cmp_to_key(lambda a,b: a[0] > b[0]))
        
        #return old,i
        #old_to_new_elems = sorted([(old,i) for (i,old) in enumerate(new_to_old_elems)], key=functools.cmp_to_key(lambda a: a[0]))
            
        #old_to_new_elems = sorted([(old,i) for (i,old) in enumerate(new_to_old_elems)], key=functools.cmp_to_key(lambda a: a[0]))
        
        #old_to_new_elems = sorted([(old,i) for (i,old) in enumerate(new_to_old_elems)], *, key=lambda a: a[0])
        # Original - old_to_new_elems = sorted([(old,i) for (i,old) in enumerate(new_to_old_elems)], lambda a,b: int.__cmp__(a[0],b[0]))
        ...
        # Copied the code from new version of ats - https://github.com/amanzi/ats/blob/master/tools/meshing_ats/meshing_ats/meshing_ats.py
